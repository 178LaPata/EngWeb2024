...
exports.compositorPage = function(compositor, d, per){
    var pagHTML = `
    <html>
    <head>
        ...
        <div class="w3-card-4">
            ...
            <div class="w3-container">
                <ul class="w3-ul w3-card-4" style="width:50%">
                    ...
                    <li><b>Periodo: </b> <a href="/periodos/${per.id}">${per.periodo}</a></li>
                </ul>
            </div>
            <footer class="w3-container w3-light-blue">
                <h5>Generated by TPC4App::EngWeb2024::a95454::${d}::<a href="/compositores">Return</a></h5>
        ...
        <body>
                ...
                <form class="w3-container" method="POST">
                    <fieldset>
                        ...
                        <div id="compositores-container">
                            ...
                            <button id="add-composer-button" type="button" onclick="addComposerField()">Adicionar Compositor</button>
                            <button id="remove-composer-button" type="button" onclick="removeComposerField()">Remover Compositor</button>
                            </div>
                    </fieldset> 
                    <br/>
                    <button class="w3-btn w3-mb-2 w3-blue-grey" type="submit" style="height:50px; display: inline-flex; justify-content: center; align-items: center; float: right;">Registar</button>
            ...
            <script>
                function addComposerField() {
                    ...
                    var addButton = document.getElementById('add-composer-button');
                    var newComposerDiv = document.createElement('div');
                    ...
                    newComposerDiv.appendChild(newComposerInput);
                    
                    container.insertBefore(newComposerDiv, addButton);
                }
                function removeComposerField() {
                    var container = document.getElementById('compositores-container');
                    var children = container.children;
                    if(children.length > 3) { // 3 because of the initial input field and the two buttons
                        var lastChild = children[children.length - 3]; // -3 to skip the two buttons and get the last input field
                        if(lastChild.firstChild.name === 'compositores') {
...
exports.periodoFormEditPage = function(a, d, comp){
    var pagHTML = `
    <!DOCTYPE html>
    <html>
        <head>
    ...
    comp.forEach(c => {
        pagHTML += `
                            <div>
                                <input class="w3-input w3-round" type="text" name="compositores" value="${c.nome}"/>
                            </div>`
    })
    pagHTML += `
                            <button id="add-composer-button" type="button" onclick="addComposerField()">Adicionar Compositor</button>
...
exports.periodoPage = function(p, d, comp){
    var pagHTML = `
    <html>
    <head>
    ...
    `
    comp.forEach(c => {
        pagHTML += `<li><a href="/compositores/${c.id}">${c.nome}</a></li>`
    })
    pagHTML += `
                </ul>
            </div>
